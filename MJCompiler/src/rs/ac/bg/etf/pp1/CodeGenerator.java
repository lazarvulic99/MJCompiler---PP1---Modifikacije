...
public class CodeGenerator extends VisitorAdaptor {
	...
	public void visit(DesignatorStatement_ternarni DesignatorStatement_ternarni) {
		report_info("INFO! Usao u CG metodu za novi ternarni!", null);
		//e1 e2 e3
		Code.put(Code.dup_x2); // e3 e1 e2 e3
		Code.put(Code.pop); // e3 e1 e2
		Code.put(Code.dup_x1); // e3 e2 e1 e2
		Code.put(Code.pop); // e3 e2 e1
		Code.loadConst(0); // e3 e2 e1 0
		Code.putFalseJump(Code.gt, 0);// e3 e2, e1 le 0
		int ucitajExpr3 = Code.pc - 2;
		//e1 != 0 => ucitaj e2
		Code.put(Code.dup_x1); // e2 e3 e2
		Code.put(Code.pop); // e2 e3
		Code.put(Code.pop); // e2
		Code.store(DesignatorStatement_ternarni.getDesignator().obj);
		Code.loadConst(0);
		Code.loadConst(0);
		Code.putFalseJump(Code.ne, 0); // 0 == 0
		int skrokKraj1 = Code.pc - 2;
		Code.fixup(ucitajExpr3);
		Code.put(Code.pop); // e3
		Code.store(DesignatorStatement_ternarni.getDesignator().obj);
		Code.loadConst(0);
		Code.loadConst(0); // 0 0
		Code.putFalseJump(Code.ne, 0); // 0 == 0
		int skokKraj2 = Code.pc - 2;
		Code.fixup(skrokKraj1);
		Code.fixup(skokKraj2);
	}
	...
	public void visit(FactorZamena_binarniNiz FactorZamena_binarniNiz) {
		report_info("Racunam vrednost binarnog niza!", null);
		Code.load(FactorZamena_binarniNiz.getDesignator().obj);
		//adr
		Code.put(Code.dup); // adr adr
		Code.put(Code.arraylength); // adr len
		Code.loadConst(0); // adr len 0
		Code.put(Code.dup_x1); // adr 0 len 0
		Code.put(Code.pop); // adr 0 len
		int petlja = Code.pc;
		Code.loadConst(-1); //adr 0 len -1
		Code.put(Code.add); //adr 0 len-1
		Code.put(Code.dup); //adr 0 len-1 len-1
		Code.loadConst(-1); //adr 0 len-1 len-1  -1
		Code.putFalseJump(Code.ne, 0); //adr 0 len-1,  len-1 == -1
		int krajPetlje = Code.pc - 2;
		Code.put(Code.dup); // adr 0 len-1 len-1
		Code.load(FactorZamena_binarniNiz.getDesignator().obj); // adr 0 len-1 len-1 adr
		Code.put(Code.dup_x1); // adr 0 len-1 adr len-1 adr
		Code.put(Code.pop); // adr 0 len-1 adr len-1
		Code.put(Code.aload); //adr 0 len-1 val(len-1)
		Code.load(FactorZamena_binarniNiz.getDesignator().obj);
		Code.put(Code.arraylength);
		Code.loadConst(-1);
		Code.put(Code.add); // adr 0 len-1 val(len-1) duzina
		Code.put(Code.dup_x2); //adr 0 duzina len-1 val(len-1) duzina
		Code.put(Code.pop); //adr 0 duzina len-1 val(len-1)
		Code.put(Code.dup_x2); //adr 0 val(len-1) duzina len-1 val(len-1)
		Code.put(Code.pop); //adr 0 val(len-1) duzina len-1
		Code.put(Code.dup_x1); // adr 0 val(len-1) len-1 duzina len-1
		Code.put(Code.sub); // adr 0 val(len-1) len-1 duzina-(len-1)
		Code.put(Code.dup_x2); // adr 0 razlika val(len-1) len-1 razlika
		Code.put(Code.dup_x1); // adr 0 razlika val(len-1) razlika len-1 razlika
		Code.put(Code.pop); //adr 0 razlika val razlika len-1
		Code.put(Code.dup_x2); // adr 0 razlika len-1 val razlika len-1
		Code.put(Code.pop); // adr 0 razlika len-1 val razlika
		Code.put(Code.shl); // adr 0 razlika len-1 novaVrednost
		Code.put(Code.dup_x2); // adr 0 novaVrednost razlika len-1 novaVrednost
		Code.put(Code.pop); // adr 0 novaVrednost razlika len-1
		Code.put(Code.dup_x1); // adr 0 novaVrednost len-1 razlika len-1
		Code.put(Code.pop); // adr 0 novaVrednost len-1 razlika
		Code.put(Code.pop); // adr 0 novaVrednost len-1
		Code.put(Code.dup_x2); //adr len-1 0 novaVrednost len-1
		Code.put(Code.pop); // adr len-1 0 novaVrednost
		Code.put(Code.add); // adr len-1 tekuciZbir
		Code.put(Code.dup_x1); // adr tekuciZbir len-1 tekuciZbir
		Code.put(Code.pop); // adr tekuciZbir len-1
		Code.putJump(petlja);
		Code.fixup(krajPetlje);
		Code.put(Code.pop);
		Code.put(Code.dup_x1);
